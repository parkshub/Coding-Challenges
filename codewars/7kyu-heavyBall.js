...
// scales.getWeight([3], [7]) returns -1 if ball 3 is heavier than ball 7, 1 if ball 7 is heavier, or 0 i these balls have the same weight. scales.getWeight([3, 4], [5, 2]) returns -1 if weight of balls 3 and 4 is heavier than weight of balls 5 and 2 etc. So where's the catch, you may ask. Well - the scales is very old. You can use it only 4 TIMES before the scale breaks.

function findBall(scales) {
  
    let answer = [0,1,2,3,4,5,6,7]
    
    while (answer.length > 1) {
  
      let a = answer.slice(0, answer.length/2)
      let b = answer.slice(answer.length/2)
      
      if (scales.getWeight(a,b) == -1) {
        answer.splice(answer.length/2)
      }
      else {
        answer.splice(0, answer.length/2)
      }
